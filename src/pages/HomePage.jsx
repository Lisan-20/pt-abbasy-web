...
const HomePage = ({ data }) => {
  const theme = data.siteSettings?.theme || {
    showHero: true,
    showWelcome: true,
    showServices: true,
    showClients: true
  ...
  return (
    <PageWrapper title="Beranda">
      {theme.showHero !== false && <Hero data={data.hero} />}
      
      {theme.showWelcome !== false && (
        <motion.div 
      ...
      {theme.showServices !== false && <Services data={data.services.slice(0, 3)} />}
      
      {theme.showClients !== false && <ClientsMarquee clients={data.clients} />}
    </PageWrapper>
  );
};
