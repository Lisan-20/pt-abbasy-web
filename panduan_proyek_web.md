...
- **Routing SPA:** Diatur menggunakan file `public/_redirects` yang berisi:
  ```text
  /assets/* /assets/:splat 200
  /* /index.html 200
  ```
  *(Sangat penting agar routing halaman dan asset CSS/JS tidak error).*
...
### A. Struktur Folder
- `src/components/`: Komponen UI yang dapat digunakan kembali (Navbar, Footer, Button).
...
- **Typografi**: Heading gunakan bobot font tebal (`fontWeight: 700` atau `800`), paragraph gunakan warna sedikit pudar untuk kontras (`color: 'rgba(255,255,255,0.7)'` di atas background gelap).
...
```
*Catatan:* Hindari menggunakan `framer-motion` untuk komponen yang melayang terus menerus (`position: fixed`) seperti *Floating WhatsApp*, karena rawan menyebabkan layout-thrashing di beberapa versi iOS.
...
- Komponen di halaman *DynamicPage* diurutkan berdasarkan properti blok dari `data.json`.

---

## 6. Integrasi Eksternal
- **WhatsApp API:** Format nomor WA harus berupa angka tanpa awalan `+` (contoh: `628123456789`).
