...
- **URL Akses:** `https://abbasyanugerahperkasa.com/admin` (atau `/admin` di localhost)
- **Login:** Menggunakan GitHub PKCE (Login with GitHub). Tidak ada sistem *email/password* pihak ketiga, 100% gratis dan aman tanpa server.
- **Konfigurasi:** File `public/admin/config.yml` dikonfigurasi untuk langsung menyimpan perubahan (commit) ke repositori GitHub `Lisan-20/pt-abbasy-web` pada *branch* `main`.
...
1. Setiap kali ada perubahan di repositori GitHub (baik melalui Git Push manual maupun simpanan dari Sveltia CMS), Cloudflare akan secara otomatis membangun ulang (*rebuild*) aplikasi.
2. **Build Settings Cloudflare:**
   - Framework preset: `Vite`
   - Build command: `npm run build`
   - Build output directory: `dist`
3. Pengaturan domain (`abbasyanugerahperkasa.com`) wajib dikelola melalui menu DNS Cloudflare, bukan dari panel pihak ketiga, dengan pengecualian *record* seperti `cpanel` dan `mail` harus diset ke **DNS Only** (Awan Abu-abu) agar fitur hosting lawas tetap berfungsi.
...
2. **`public/robots.txt`**: Berisi perintah `Disallow: /admin` yang melarang mesin pencari Googlebot untuk menelusuri halaman admin.
3. **Meta Noindex**: Halaman `admin/index.html` dilengkapi dengan Meta Label `noindex, nofollow` agar 100% tidak terindeks oleh Google.
4. **`vite.config.js` (Obfuscation)**: Diprogram secara khusus agar setiap kali `npm run build` dijalankan, ia akan menghancurkan *Source Maps* (agar struktur *file* disembunyikan dari publik) dan secara otomatis menghapus seluruh jejak `console.log` peninggalan masa *development*.
