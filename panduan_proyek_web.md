...
- **Site Name (Nama Situs Resmi)**: Agar Google tidak menampilkan nama *domain* mentah, Anda wajib menanamkan tipe skema `WebSite` di *file* utama dan juga tag meta `<meta property="og:site_name" content="Nama Perusahaan" />` pada *file* `index.html` dan `PageWrapper.jsx`.
- **Open Graph (OG)**: Sisipkan `<meta property="og:image"...>` di `index.html` untuk memunculkan gambar saat URL dibagikan.
...
- Jangan lupa daftarkan `sitemap.xml`. Jika ikon atau nama situs tidak langsung berubah, jangan panik; **Google membutuhkan waktu 3-14 hari** (fenomena Google Dance) untuk memantapkan posisi pencarian Anda!
