...
Anda dapat mengubah data tersebut secara manual melalui file JSON, ATAU melalui Dasbor Admin CMS.

## 2. Dasbor Admin (Sveltia CMS)
Situs ini telah dimigrasikan dari Netlify (Decap CMS) menjadi 100% Serverless menggunakan **Sveltia CMS**.
- **URL Akses Rahasia:** `https://abbasyanugerahperkasa.com/manajemen-web` (Gembok Javascript dari domain utama telah dicabut, dan nama folder diubah untuk keamanan/Security through Obscurity).
- **Metode Login (VIP):** Menggunakan **GitHub Personal Access Token (PAT)**. Tidak menggunakan server penengah pihak ketiga, murni terhubung dari *browser* klien ke pusat data GitHub.
- **Konfigurasi:** File `public/manajemen-web/config.yml` dikonfigurasi untuk langsung menyimpan perubahan ke repositori GitHub `Lisan-20/pt-abbasy-web` pada *branch* `main`.

## 3. Hosting & Deployment (Cloudflare Pages)
Proses *deployment* (penyebaran) 100% di-hosting secara gratis menggunakan infrastruktur militer dari **Cloudflare Pages**.
1. Setiap kali ada perubahan di repositori GitHub (baik melalui Git Push manual maupun simpanan dari Dasbor CMS), Cloudflare akan secara otomatis membangun ulang (*rebuild*) aplikasi.
2. **Build Settings Cloudflare:**
   - Framework preset: `Vite`
   - Build command: `npm run build`
   - Build output directory: `dist`
3. Pengaturan domain (`abbasyanugerahperkasa.com`) wajib dikelola melalui menu DNS Cloudflare, bukan dari panel cPanel/Rumahweb, dengan pengecualian *record* seperti `cpanel` dan `mail` harus diset ke **DNS Only** (Awan Abu-abu) agar fitur *hosting* lawas tetap berfungsi.

## 4. Keamanan dan Optimalisasi (Tingkat Tinggi)
Proyek ini telah dikonfigurasi dengan standar keamanan tertinggi:

1. **`public/_headers` (Perisai Cloudflare)**: Menyimpan instruksi keamanan seperti `X-Frame-Options: DENY` untuk mencegah serangan *Clickjacking* dan *XSS*.
2. **Meta Noindex**: Halaman `manajemen-web/index.html` dilengkapi dengan Meta Label `noindex, nofollow` agar 100% tidak terindeks oleh Google bot.
3. **`vite.config.js` (Obfuscation)**: Diprogram secara khusus agar setiap kali `npm run build` dijalankan, ia akan menghancurkan *Source Maps* (agar struktur *file* disembunyikan dari publik) dan secara otomatis menghapus seluruh jejak `console.log` peninggalan masa *development*.

## 5. SEO Meta dan JSON-LD (Search Engine Optimization)
